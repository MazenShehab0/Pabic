...
/* EVENTS: load from events.json + fallback next Saturday */
(async () => {
  const elDate = document.getElementById("eventDate");
  const elBand = document.getElementById("eventBand");
  const elNote = document.getElementById("eventNote");
  if(!elDate || !elBand || !elNote) return;

  const fmt = (d) => {
    const opts = { weekday:"long", year:"numeric", month:"long", day:"numeric" };
    return d.toLocaleDateString(document.documentElement.lang === "en" ? "en-GB" : "sr-RS", opts);
  };

  const nextSaturday = () => {
    const now = new Date();
    const day = now.getDay(); // 0 Sun ... 6 Sat
    const add = (6 - day + 7) % 7 || 7; // next Saturday (not today)
  ...
  const lang = localStorage.getItem("pabic_lang") || "sr";
  const d = (data?.next?.date) ? new Date(data.next.date + "T12:00:00") : nextSaturday();
  elDate.textContent = fmt(d);

  const band = (lang === "en") ? (data?.next?.band_en || "TBA") : (data?.next?.band_sr || "TBA");
  const note = (lang === "en") ? (data?.next?.note_en || "Starts around 21:30") : (data?.next?.note_sr || "Početak oko 21:30");
  elBand.textContent = band;
  elNote.textContent = note;
})();

/* POPUP: show once (and remember) */
(() => {
  const backdrop = document.getElementById("promoBackdrop");
  const closeBtn = document.getElementById("popClose");
  if(!backdrop || !closeBtn) return;

  const KEY = "pabic_promo_dismissed_at";
...
(() => {
  ...
  const items = Array.from(document.querySelectorAll(".gallery-grid a.gcard img"))
  ...
  // intercept clicks
  document.querySelectorAll(".gallery-grid a.gcard").forEach((a) => {
    const im = a.querySelector("img");
    if(!im) return;                 // skip wide card
